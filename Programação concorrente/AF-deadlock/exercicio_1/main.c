...
sem_t sem_lista;
//estava com problema de espera circular, então precisei apenas que a ordem global de pegar os semáforos e mutex fosse a mesma para os dois workers

//worker1 segurar o semáforo e estiver esperando pelo mutex enquanto worker2 segurar o mutex e estiver esperando pelo semáforo, isso causará um deadlock.
void *worker1_func(void *arg) {
    for (int i = 0; i < WORKER_LOOPS; ++i) {
        pthread_mutex_lock(&mutex_total); //obtem o mutex depois
        printf("Worker 1 obteve mutex_total\n");
        sem_wait(&sem_lista); //decrementa e trava  (obtem o semafoto primeiro)
