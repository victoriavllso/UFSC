...
void structures::LinkedList<T>::invert() {
    ...
    for (size_t i = 0; i < size(); i++) {  // enquando não chegar no ultimo nó
        proximo = atual->next();
        atual->next(anterior);
        anterior = atual;
        atual = proximo;
...
structures::LinkedList<T> structures::LinkedList<T>::clone() {
    ...
    for (size_t i = 0; i < size(); i++) {
        list_clone.insert(atual->data(), i);
        atual = atual->next();
    }
    return list_clone;
}
...
structures::LinkedList<T> structures::LinkedList<T>::slicing(int start,
    ...
    // COLOQUE SEU CODIGO AQUI...
    // ...
    if (!empty() && start >= 0 && stop >0) {
    for (int i = start; i <= stop; i += step) {
cod        list_slice.push_back(at(i));  // por que não funcionava com insert ?
    }
    } else {
        throw std::out_of_range("lista vazia");
    }
...
void structures::LinkedList<T>::append(structures::LinkedList<T> *list_add) {
    ...
    for (size_t i = 0; i < list_add->size(); i++) {
        insert(list_add->at(i), tamanho +i);
    }
}
...
structures::LinkedList< structures::LinkedList<T> * >
    ...
    Node *atual = head;
    LinkedList<T> *sublistpar = new LinkedList<T>();
    LinkedList<T> *sublistimpar = new LinkedList<T>();
    for (size_t i =0; i < size(); i++) {
        if (i % 2 == 0) {
            sublistpar->push_back(atual->data());
        } else {
            sublistimpar->push_back(atual->data());
        }
        atual = atual->next();
    }
    list_halve.push_front(sublistpar);
    list_halve.push_back(sublistimpar);
    return list_halve;
}
