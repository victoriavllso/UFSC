...
class MazeGame(Game):
    def __init__(self, settings=GameSettings(), ranking=RankingManager(), viewstate=None,screen=ScreenBase):
        ...
        # colocar na singleton e usar nomes mais claros nas variaveis
        self.__iw = (self.settings.get_width() - (block_size*2) * s)//2
    ...
    def render_player_life(self):
        ...
        self.get_graphics_api().draw_2d_text(life_txt, self.settings.get_width() //
                                             1.6 + 100, 30, (255, 255, 255), (0, 0, 0), font_size=24)

    def loop(self, event=None):
        if self.get_world().pause:
            return
        time_at = time.time()
    ...
    def create_maze_map(self):
        ...
        self.__plocal.set_position(
            Vector3(self.__iw+block_size, self.__ih+block_size, 0))
        ...
        def end_pos(x): return s * block_size*2 + \
            (self.__iw if x == "x" else self.__ih - block_size)
        flag = EndMazeFlag(initial_position=Vector3(
            end_pos('x'), end_pos('y'), 0), collision_polygons=[Square(size=15)])
        self.get_world().add_object(flag)

    def generate_random_power_up(self):
        PowerUp = random.choice([PowerUpLife, PowerUpSpeed])
        random_pos = self.__maze.get_random_free_position(
            margin=Vector3(self.__iw, self.__ih), exclude=self.__actor_up_pos)
        self.__actor_up_pos.add(random_pos.get_float_tuple_2d())
        power_up = PowerUp(initial_position=random_pos, collision_polygons=[
                           Square(self.settings.get_block_size())])
    ...
    def generate_random_obstacle(self):
        ...
        random_pos = self.__maze.get_random_free_position(
            margin=Vector3(self.__iw, self.__ih), exclude=self.__actor_up_pos)
        self.__actor_up_pos.add(random_pos.get_float_tuple_2d())
        obstacle = Obstacle(initial_position=random_pos, collision_polygons=[
                            Square(self.settings.get_block_size())])
